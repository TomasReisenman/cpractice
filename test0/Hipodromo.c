...
int inicializar_pistas(pista pistas[],int cantidad,int metrosCarrera)
{

	int i;
	for(i = 0;i<cantidad;i++)
	{
		pistas[i].numeroCaballo = i + 1;	
		pistas[i].metrosRecorridos = 0;	
		pistas[i].metrosAlDisco = metrosCarrera;	
		pistas[i].accion = 1;	
	}
...
int  verificar_estado_caballos(pista pistas[],int cantidad,int metrosCarrera)
{
	int ganador = 0;
	...
	for(i = 0;i<cantidad;i++){
		...
		if(pistas[i].accion == 0)
		{
			pistas[i].metrosAlDisco = metrosCarrera - pistas[i].metrosRecorridos;

			if(metrosCarrera > pistas[i].metrosRecorridos)
			{
				pistas[i].accion = 1;	
			...
			}else
			{
				...
				ganador = pistas[i].numeroCaballo;
				break;
...
int main(int argc,char *argv[])
{
	...
	int metrosCarrera = obtener_numero_aleatorio(800,1000);
	printf("Largo de la carrera es %d \n",metrosCarrera);
	int hay_ganador = 0;
	...
	inicializar_pistas(pistas,cantidad_caballos,metrosCarrera);
	...
	while(!hay_ganador)
	{
		...
		hay_ganador = verificar_estado_caballos(pistas,cantidad_caballos,metrosCarrera);
